...
impl Insurance {
    ...
    pub fn set_pause_admin(env: Env, caller: Address, new_admin: Address) -> Result<(), InsuranceError> {
        ...
        match current {
            ...
            Some(ref admin) if admin != &caller => return Err(InsuranceError::Unauthorized),
            _ => {}
        }
        env.storage()
            .instance()
    ...
    pub fn set_upgrade_admin(env: Env, caller: Address, new_admin: Address) -> Result<(), InsuranceError> {
        ...
        match current {
            ...
            Some(ref adm) if adm != &caller => return Err(InsuranceError::Unauthorized),
            _ => {}
        }
        env.storage()
            .instance()
    ...
    pub fn create_policy(
        ...
        policies.set(next_id, policy.clone());
        env.storage()
            .instance()
            .set(&symbol_short!("POLICIES"), &policies);
        env.storage()
    ...
    pub fn get_active_policies(env: Env, owner: Address, cursor: u32, limit: u32) -> PolicyPage {
        ...
        for (_, policy) in policies.iter() {
            if policy.active && policy.owner == owner {
                if cursor == 0 || found_cursor {
                    if items.len() < limit as usize {
                        items.push_back(policy);
                    } else {
                        break;
                    }
                } else if policy.id == cursor {
                    found_cursor = true;
                }
                if !found_cursor && cursor > 0 {
                    start_index += 1;
                }
            }
        }
        ...
        let next_cursor = if items.len() == limit as usize {
            items.last().map(|p| p.id).unwrap_or(0)
        } else {
            0
        };
